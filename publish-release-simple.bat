...
echo.
echo ================================================
echo     FOUNDRY REST API - ONE-CLICK RELEASE
echo ================================================
echo.
...
for /f "tokens=2 delims=:" %%a in ('findstr /c:"\"version\"" src\module.json') do (
    set version_line=%%a
...
if "%skip_version_update%"=="0" (
    ...
    REM This is tricky in batch, so we'll use PowerShell
    powershell -Command "(Get-Content 'src\module.json') -replace '\"version\": \".*\"', '\"version\": \"%new_version%\"' | Set-Content 'src\module.json'"
...
REM 1. Build the module
echo [1/6] Building module...
call npm run build
if %errorlevel% neq 0 (
...
echo   - release\foundry-rest-api-v%new_version%.zip
echo.
pause